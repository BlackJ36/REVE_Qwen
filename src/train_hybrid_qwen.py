"""Plan B training: HybridEncoder + Qwen3-0.6B full fine-tune.

2 LR groups: encoder projector (1e-3), Qwen full model (2e-5).
Reuses BCIE2EDataset and BCIE2EDataCollator directly.
"""
...
from .model_hybrid_qwen import build_hybrid_qwen_model


class HybridQwenTrainer(Trainer):
    """Trainer with 2 LR groups for Qwen full fine-tune.

    LR groups:
        - Encoder projector: projector_lr (default 1e-3)
        - Qwen: args.learning_rate (default 2e-5)
    """
    ...
    def create_optimizer(self):
        ...
        if qwen_params:
            optimizer_grouped_parameters.append(
                {"params": qwen_params, "lr": self.args.learning_rate}
            )
        ...
        print(f"HybridQwen Optimizer ({len(optimizer_grouped_parameters)} groups):")
        print(f"  Projector: {len(projector_params)} params, lr={self.projector_lr}")
        print(f"  Qwen:      {len(qwen_params)} params, lr={self.args.learning_rate}")
    ...
    def _save_weights(self, output_dir):
        ...
        torch.save(
            self.model.encoder.projector.state_dict(),
            output_dir / "encoder_projector.pt",
        )
        ...
        meta = {"model_type": "hybrid_qwen", "model_name": "Qwen3-0.6B"}
        with open(output_dir / "hybrid_qwen_meta.json", "w") as f:
            json.dump(meta, f)


def run_hybrid_qwen_training(
    ...
    """Plan B training entry point."""
    print("Building Hybrid Qwen model...")
    model, tokenizer = build_hybrid_qwen_model(
    ...
    qwen_p = sum(p.numel() for n, p in model.named_parameters() if p.requires_grad and "encoder.projector" not in n)
    print(f"\nTrainable parameters:")
    print(f"  Encoder projector: {proj_p:>12,}  (lr={projector_lr})")
    print(f"  Qwen (full):       {qwen_p:>12,}  (lr={learning_rate})")
    print(f"  Total:             {proj_p + qwen_p:>12,}\n")
